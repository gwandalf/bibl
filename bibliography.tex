...
Avant tout, il faut définir ce qu'est une interaction. L'article \cite{margery} la définit comme étant le changement de valeur d'un gestionnaire d'interaction (GI). Un GI est matérialisé dans l'EV par une poignée ou une boule par exemple. En particulier, les points de contact sont  des GI permettant de changer la valeur des coordonnées d'un point. D'autres GI peuvent par exemple permettre de changer la valeur de la couleur ou de la transparence d'un objet. Ainsi les interactions peuvent être associées à des événements ordonnés dans le temps.
\\
...
La deuxième propriété s'obtient grâce à un mécanisme d'inscription / désinscription au moment où un utilisateur commence à interagir avec un objet (resp. termine son interaction). A chaque interaction, l'objet considère que tous les utilisateurs inscrits sont en activité simultanée et opère d'abord la diffusion d'un message contenant le nombre d'inscrits, puis diffuse les valeurs des gestionnaires d'interaction. Chaque processus attend donc le nombre d'interactions correspondant au nombre reçu au préalable avant de calculer la transformation résultante.
